...
ENT.PrintName = "Covenant Datapad"
ENT.Author = "Fiorelus"
...
function ENT:Initialize()
    if SERVER then
        self:SetModel("models/impulse/halo/props/covenant/cov_holograms/cov_touchscreen_03.mdl")
...
function ENT:Use(activator, caller)
    if IsValid(caller) and caller:IsPlayer() and self:GetClass() == "ix_cov_datapad" and not self.IsUsing then
        self.IsUsing = true
        local terminalID = self:GetTerminalID()
        net.Start("OpenDatapadUI")
...
properties.Add("datapad2_id", {
    ...
    Filter = function(self, ent, ply)
        ...
        if ent:GetClass() ~= "ix_cov_datapad" then return false end
        return true
    end,
    ...
    Receive = function(self, length, ply)
        ...
        if IsValid(ent) and ent:GetClass() == "ix_cov_datapad" then
