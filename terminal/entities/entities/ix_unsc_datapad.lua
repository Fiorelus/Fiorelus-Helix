...
function ENT:Initialize()
    if SERVER then
        self:SetModel("models/ishi/halo_rebirth/props/human/tac_pad.mdl")
...
function ENT:Use(activator, caller)
    if IsValid(caller) and caller:IsPlayer() and self:GetClass() == "ix_unsc_datapad" and not self.IsUsing then
        self.IsUsing = true
...
properties.Add("datapad1_id", {
    ...
    Filter = function(self, ent, ply)
        ...
        if ent:GetClass() ~= "ix_unsc_datapad" then return false end
        return true
    end,
    ...
    Receive = function(self, length, ply)
        ...
        if IsValid(ent) and ent:GetClass() == "ix_unsc_datapad" then
